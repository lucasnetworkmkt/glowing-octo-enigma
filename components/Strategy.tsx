...
const Strategy: React.FC = () => {
  return (
    <section id="strategy" className="py-16 md:py-24 bg-brand-blue text-white relative border-t border-brand-accent/5">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {strategies.map((item, index) => {
            ...
            return (
              <div key={index} className="bg-brand-blue/50 border border-brand-accent/10 p-6 md:p-8 rounded-2xl hover:border-brand-accent/50 hover:bg-brand-accent/5 hover:shadow-[0_0_20px_rgba(0,212,255,0.1)] transition-all duration-300 group">
                <div className="w-10 h-10 md:w-12 md:h-12 bg-brand-accent/10 rounded-lg flex items-center justify-center mb-4 md:mb-6 group-hover:bg-brand-accent group-hover:scale-110 transition-all duration-300">
                  <Icon className="w-5 h-5 md:w-6 md:h-6 text-brand-accent group-hover:text-brand-blue" />
                </div>
                <h3 className="text-lg md:text-xl font-heading font-bold mb-2 md:mb-3 group-hover:text-brand-accent transition-colors">{item.title}</h3>
